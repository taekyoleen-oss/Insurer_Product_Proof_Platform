...
--   2. 생성된 UUID를 아래 SUPER_ADMIN_USER_ID에 입력하세요.
--   3. SQL Editor에서 이 파일 전체를 실행하세요.
-- ============================================================

DO $$
DECLARE
  -- ★ 아래 UUID를 실제 super_admin 계정의 user_id로 교체하세요 ★
  super_admin_uid   uuid := '00000000-0000-0000-0000-000000000000';
...
VALUES (
  'hazard_rate',
  '2025년 하반기 사망·재해 위험률 검증',
  '표준생명표 2025년 개정을 위한 사망 및 재해 위험률 종합 검증.',
  agency1_id, 'completed',
  ARRAY['사망위험률', '상해위험률'],
  '2025-12-15'::date,
  '2025-10-01',
  '2025-11-28',
...
-- ============================================================
-- 3. 코멘트 샘플 (진행중 건에 첨부)
--    author_id는 super_admin_uid 사용 (실제 운영 시 각 담당자 ID로 대체)
-- ============================================================

INSERT INTO ippp_comments (request_id, author_id, content)
VALUES
...
RAISE NOTICE '   - 코멘트: 8개';
RAISE NOTICE '';
RAISE NOTICE '⚠️  super_admin_uid가 더미값(000...)으로 설정되어 있습니다.';
RAISE NOTICE '    실제 super_admin UUID로 교체 후 실행하거나,';
RAISE NOTICE '    created_by/author_id 컬럼은 NULL로 설정해도 무방합니다.';
